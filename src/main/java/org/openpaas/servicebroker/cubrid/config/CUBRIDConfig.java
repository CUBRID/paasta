...
public class CUBRIDConfig {
	...
	public JSchUtil jschUtil() {
		String host = this.env.getRequiredProperty("docker.host");
		int port = Integer.parseInt(this.env.getRequiredProperty("docker.port"));
		String username = this.env.getRequiredProperty("docker.userName");
		String password = this.env.getRequiredProperty("docker.password");
		String privateKey = this.env.getRequiredProperty("docker.privateKey");
